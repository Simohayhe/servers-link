    ...
    public synchronized void send(Packet packet) {
        if (executor.isShutdown()) {
            ...
            try {
                executor.submit(() -> {
                    try {
                        // GatewayインスタンスのNullチェックを追加
                        if (Gateway.getInstance() != null && Gateway.getInstance().isDebugEnabled()) {
    ...
    public void run() {
        try {
            ...
            while (IS_RUNNING) {
                ...
                if (!(obj instanceof Packet packet)) continue;
                ...
                if (packet instanceof NewServerPacket pkt) {
                    ...
                    Group group = Gateway.getInstance().getGroup(server.getGroupId());
                    if (group != null) {
                    ...
                    Group globalGroup = Gateway.getInstance().getGroup("global");
                    if (globalGroup != null) {
                        Settings globalSettings = globalGroup.getSettings();
                ...
                // 修正ポイント：serverがnullの場合は"Unknown"として扱う
                final String currentServerName = (this.server != null) ? this.server.getName() : "Unknown";
                SERVER.execute(() -> packet.onGatewayReceive(currentServerName));
            }
            socket.close();
        } catch (IOException e) {
            String serverName = (this.server != null) ? this.server.getName() : "Unknown";
            if (e.getMessage() != null) {
                ServersLink.LOGGER.error("Error {} in sub-server {}", e.getMessage(), serverName);
            }
            if (this.server != null) {
                ServersLinkApi.disconnectServer(this.server);
                ServersLinkApi.broadcastToOp(Text.literal("Sub-server " + serverName + " has disconnected").formatted(Formatting.RED));
            }
            this.interrupt();
