...
public class SubServer extends Thread {
    ...
    @Override
    public void run() {
        // 追加: 接続に失敗している（in が null）場合は何もしない
        if (in == null) {
            ServersLink.LOGGER.warn("SubServer thread stopped: Connection was not established.");
            return;
        }
        ...
        try {
            ...
            while (IS_RUNNING) {
                try {
                    Object obj = in.readObject(); // 一旦 Object で受ける
                    if (obj instanceof Packet pkt) {
            ...
            // サーバー終了時などでソケットが閉じられた際の予期せぬエラーを防ぐ
            if (IS_RUNNING) {
                ServersLink.LOGGER.error("Gateway disconnected");
                SERVER.stop(false); // trueからfalseへ（再帰的な停止エラー防止）
            }
        }
    }
