...
public class Gateway extends Thread {
    ...
    private boolean globalPlayerCount = false;
    private boolean whitelistIp = false;
    private final List<String> whitelistedIp = new ArrayList<>();
    private boolean reconnectLastServer = false;

    public Gateway(int port) {
        // 先に設定を読み込んで変数とインスタンスを初期化（NullPointerException防止）
        gateway = this;
        loadConfig();
        groups = new HashMap<>();
        ...
        try {
            ...
            ServersLink.LOGGER.error("Unable to start central server (Port {} may be in use): {}", port, e.getMessage());
            // serverSocketはnullのままになるが、run()のチェックで安全に止まる
        }
    }
        ...
        if (serverName.equals(ServersLink.getServerInfo().getName())) {
            ...
            for (ServerInfo server : ServersLinkApi.getServerList()) {
                if (server.getName().equals(serverName)) {
                    G2SConnection conn = ServersLinkApi.getServerMap().get(server);
    ...
    public void forward(Packet packet, String sourceServer) {
        ServerInfo sourceInfo = ServersLinkApi.getServer(sourceServer);
        if (sourceInfo == null) return;

        String sourceGroup = sourceInfo.getGroupId();
        for (ServerInfo server : ServersLinkApi.getServerList()) {
    ...
    public Settings getSettings(String sourceGroup, String destinationGroup) {
        ...
        if (a != null && a.getRules().containsKey(destinationGroup)) {
            return a.getRules().get(destinationGroup);
        } else if (a != null && sourceGroup.equals(destinationGroup)) {
            return a.getSettings();
        } else {
            Group global = groups.get("global");
            return global != null ? global.getSettings() : new Settings(false, false, false, false, false);
        }
    }
    ...
    public void loadConfig() {
        ...
        if (!Files.exists(path)) {
            ServersLink.LOGGER.warn("config.json not found, using default gateway settings.");
            return;
        }
        ...
        try {
            ...
            if (jsonObject != null) {
                if (jsonObject.has("debug")) debug = jsonObject.get("debug").getAsBoolean();
                if (jsonObject.has("global_player_count")) globalPlayerCount = jsonObject.get("global_player_count").getAsBoolean();
                if (jsonObject.has("whitelist_ip")) whitelistIp = jsonObject.get("whitelist_ip").getAsBoolean();
                if (jsonObject.has("whitelisted_ip")) {
                    whitelistedIp.clear();
                    for (JsonElement element : jsonObject.getAsJsonArray("whitelisted_ip")) {
                        whitelistedIp.add(element.getAsString());
                    }
                }
                if (jsonObject.has("reconnect_last_server")) reconnectLastServer = jsonObject.get("reconnect_last_server").getAsBoolean();
            }
        } catch (IOException e) {
    ...
    private void loadGroups() {
        ...
        if (!Files.exists(path)) {
            ServersLink.LOGGER.warn("groups.json not found.");
            return;
        }
                ...
                for (int i = 0; i < ruleGroups.size(); i++) {
                    ...
                    for (int j = 0; j < ruleGroups.size(); j++) {
                        if (i != j) {
                            ...
                            if (g != null) g.addRule(ruleGroups.get(j).getAsString(), ruleSettings);
                        }
                    }
                }
            }
        } catch (Exception e) {
            ServersLink.LOGGER.error("Unable to read groups.json: {}", e.getMessage());
        }
    }

    @Override
    public void run() {
        // 安全チェック: サーバーソケットが作られていなければスレッドを停止
        if (serverSocket == null) {
            ServersLink.LOGGER.error("Gateway ServerSocket is null. Stopping Gateway thread.");
            return;
        }
        ...
        while (IS_RUNNING) {
            try {
                ...
                if (IS_RUNNING) {
                    ServersLink.LOGGER.info("Unable to accept connection: {}", e.getMessage());
                }
            }
        }
    }
